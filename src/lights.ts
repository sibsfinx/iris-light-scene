...
export function setupLights(scene: THREE.Scene): void {
  ...
  const key = new THREE.PointLight(0xc8deff, 40, 12, 1.2);
  key.position.set(3.0, 4.5, 1.2);
  scene.add(key);

  // Upper-left fill — cooler, weaker
  const fill = new THREE.PointLight(0x8aaae0, 22, 10, 1.2);
  fill.position.set(-2.8, 4.0, 0.6);
  scene.add(fill);

  // Back rim — subtle back-light
  const rim = new THREE.PointLight(0xb0ccff, 14, 7, 1.6);
  rim.position.set(1.8, 0.8, -2.8);
  scene.add(rim);

  // Ambient fill — keeps shadowed petals from going pure black
